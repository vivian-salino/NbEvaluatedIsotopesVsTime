...
# ENDF/B-II
tail -n +2 B-II/bnlendfb2tape20*.txt | cut -c 67-70 > cut.txt
...
#  Beyond neutron data and as laid out by https://www.nndc.bnl.gov/endf-b6.8/summary.html, it also includes:
...
# That portion of script (a bit long to execute ; can be commented) allows to find
# the tapes that contain a specific MAT (for all MATs)
...
#            if tail -n +2 B-VI/tape.$tape | grep -q -E '^.{66} '$MAT; then
#                echo "tape."$tape >> TapeForEachMAT.txt
#            fi
#            #tail -n +2 B-VI/tape.$tape | grep --color=None -E '^.{66} '$MAT
...
do
    ...
    do
        ...
        for tape in ${!tapes}
        do
            tail -n +2 B-VI/tape.$tape | cut -c 67-70 >> cut.txt
        done
    done
    NbIso=$(sort -un cut.txt | sed 's/ //g' | awk '{if(length() >= 3) print $0}' | wc -l)
    rm -f cut.txt
...
# ENDF/B-VII.0
echo "NbIso['ENDF/B-VII.0'] = "$(ls B-VII.0 | wc -l)
# ENDF/B-VII.1
echo "NbIso['ENDF/B-VII.1'] = "$(ls B-VII.1 | wc -l)
# ENDF/B-VIII.0
echo "NbIso['ENDF/B-VIII.0'] = "$(ls B-VIII.0 | wc -l)
# ENDF/B-VIII.1
echo "NbIso['ENDF/B-VIII.1'] = "$(ls B-VIII.1/*.endf | wc -l)
